...
--> statement-breakpoint
INSERT INTO `__new_user`("id", "name", "email", "email_verified", "image", "created_at", "updated_at", "locale", "handle") SELECT "id", "name", "email", "email_verified", "image", "created_at", "updated_at", "locale", "handle" FROM `user`;--> statement-breakpoint
DROP TABLE `user`;--> statement-breakpoint
ALTER TABLE `__new_user` RENAME TO `user`;--> statement-breakpoint
CREATE UNIQUE INDEX `user_email_unique` ON `user` (`email`);--> statement-breakpoint
-- Hand-written: drizzle-kit's user-table rebuild above (triggered by the new column defaults, not
-- by anything handle-related) drops every index on `user`, including the hand-written partial
-- unique index from 0002_melted_guardsmen.sql that isn't modeled in auth-schema.ts (see that
-- migration's comment for why). Recreate it here so setUserHandle's uniqueness check keeps working.
CREATE UNIQUE INDEX `user_handle_unique` ON `user` (`handle`) WHERE `handle` IS NOT NULL;--> statement-breakpoint
