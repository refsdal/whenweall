...
CREATE UNIQUE INDEX `booking_pages_org_slug_uidx` ON `booking_pages` (`organization_id`,`slug`) WHERE "booking_pages"."deleted_at" IS NULL;--> statement-breakpoint
...
CREATE TABLE `polls` (
	...
	`signup_max_claims` integer DEFAULT 1 NOT NULL,
	`created_at` text NOT NULL,
	`updated_at` text NOT NULL,
	`deleted_at` text,
	FOREIGN KEY (`organization_id`) REFERENCES `organization`(`id`) ON UPDATE no action ON DELETE cascade,
...
--> statement-breakpoint
CREATE INDEX `polls_org_created_idx` ON `polls` (`organization_id`,`created_at`);--> statement-breakpoint
CREATE TABLE `votes` (
